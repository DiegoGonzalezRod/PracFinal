...
export async function updateDeliveryNote(deliveryNoteId, deliveryNoteData) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorDetails.message || "Error al actualizar el albarán");
    }

    return await response.json();
  } catch (error) {
    console.error("Error al actualizar el albarán:", error.message);
    throw error;
  }
}
...
export async function getDeliveryNoteById(id) {
  ...
  if (!response.ok) throw new Error("Error al obtener el albarán.");
  return await response.json();
}
